...
function addGame(game, isFirstChild) {
    const tableBodyElm = document.querySelector(".games-table--body")
    const gameRowTemlate = `
        <tr class="row">
            <td><span class="game-id">${game.id}</span></td>
            <td><span class="${game.isFinished? "game-status__finished" : ""}" >${game.isFinished ? "Finished" : ""}</span></td>

            <td><input class="table-input table-input__name" type="text" value="${game.date}" placeholder="Name"></td>

            <td>
                <div class="game-team">
                    <div class="team-img"><img src="${APP.HOSTNAME + game.team1.logo}"></div>
                    <input class="table-input table-input__captain" type="text" value="${game.team1.name}">
                </div>
            </td>
            ...
            <td>
                <div class="game-team">
                    ...
                    <input class="table-input table-input__captain" type="text" value="${game.team2.name}">
                </div>
            </td>

            <td><input class="table-input table-input__captain" type="text" value="${game.league.name}"></td>

            <td><input class="table-input table-input__captain" type="text" value="${game.description || ""}" placeholder="description"></td>
    ...
    gameRowElem.querySelector(".dropdown-link_delete").addEventListener("click", () => {
        ...
        APP.fetch(`game/${team.id}`, { method: "DELETE", actor: "admin" }).then(result => {
            waitNot.pop()
            if (result.success) {
...
async function gamesTabInit() {
    ...
    const teamsResult = await APP.fetch("game", { query: { include: "team,league" } })
    const gamesData = teamsResult.data
    ...
    // add fetched team to the table
    gamesData.forEach(game => addGame(game))
